    ...
    <td>
        ...
        <?php if($type_price == 'people_price'){ ?>
            <p><strong><?php _e('Adult Price',ST_TEXTDOMAIN) ?>: </strong>
                ...
                <?php echo TravelHelper::format_money(get_post_meta($order_id,'adult_price',true));?>
            </p>
            <p><strong><?php _e('Child Price',ST_TEXTDOMAIN) ?>: </strong>
                <?php echo get_post_meta($order_id,'child_number',true);?> x
                <?php echo TravelHelper::format_money(get_post_meta($order_id,'child_price',true));?>
            </p>
        <?php }else{ ?>
        ...
        <p><strong><?php st_the_language('tour_date') ?> : </strong>
            <?php $type_tour = get_post_meta($object_id,'type_tour',true); ?>
            <?php if($type_tour == 'specific_date'){ ?>
                <?php if($check_in) echo date_i18n('m/d/Y',strtotime($check_in)); ?>
                -
                <?php if($check_out) echo date_i18n('m/d/Y',strtotime($check_out)); ?>
            <?php }else{ ?>
                <?php if($check_in) echo date_i18n('m/d/Y',strtotime($check_in)); ?>
                <p><strong>
