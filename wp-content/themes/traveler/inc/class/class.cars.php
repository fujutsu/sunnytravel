...
SELECT st_meta6.meta_value as booked_id ,st_meta2.meta_value as check_in,st_meta3.meta_value as check_out
        ...
        function do_add_to_cart()
        {
            ...
            //from 1.1.4 update whenever remove check_in in Get method
            $check_in_n   =(TravelHelper::convertDateFormat($data_price_cars->date_time->pick_up_date));
            $check_out_n  =(TravelHelper::convertDateFormat($data_price_cars->date_time->drop_off_date));
            ...
            $check_in_unix = TravelHelper::convertDateFormat( $data_price_cars->date_time->pick_up_date );
            
            if(isset($_REQUEST['pick-up-date'])){
                $check_in_unix = TravelHelper::convertDateFormat($_REQUEST['pick-up-date']);
            }
            ...
            $period = TravelHelper::dateDiff($today,$check_in_unix);
            $compare = TravelHelper::dateCompare($today, $check_in_unix);
        ...
        function change_search_cars_arg( $query )
        {
            ...
            if($query->is_search && $post_type == 'st_cars') {
                ...
                if($location_id = STInput::get( 'location_id_pick_up' )) {
                    $ids_in  = array();
