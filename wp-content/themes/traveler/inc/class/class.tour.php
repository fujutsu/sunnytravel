...
//                                )";
            ...
            function change_search_tour_arg($query)
            {
                ...
                if($query->is_search && $post_type == 'st_tours')
                {
                    ...
                    add_filter('posts_join', array($this, '_alter_join_query'));
            ...
            */
            function filter_price_by_person($price_old, $number ,  $key = 1 ){
                // after set "Fields list discount by Adult number booking" in admin
                // key 1 : adult, key 2 : child
            ...
            function do_add_to_cart()
            {
                ...
                $max_booking_number = get_post_meta($item_id , 'max_people' , true);
                if(!empty($max_booking_number) and $max_booking_number < $number ){
                    STTemplate::set_message(__('Exceed the maximum of allowed number',ST_TEXTDOMAIN),'danger');
                    return ;
                }
                ...
                if($data['type_price']=='people_price')
                {
                    ...
                    $prices=self::get_price_person($item_id,$tinhsoluong=true);     

                    
                    $data['adult_price']=(!empty($prices['adult_new']))? $prices['adult_new'] : $prices['adult'];
                    $data['child_price']=(!empty($prices['child_new']))? $prices['child_new'] : $prices['child'];
                    $data['discount']= $discount;
                    $data['adult_number']=STInput::request('adult_number',1);
                    $data['child_number']=STInput::request('children_number',0);
                    
                    $data['adult_price'] = $this->filter_price_by_person($data['adult_price'], $data['adult_number'] ,  1 );
                    $data['child_price'] = $this->filter_price_by_person($data['child_price'], $data['child_number'] ,  2 ); 
                ...
                if($type_tour == 'daily_tour'){
                    ...
                    $duration_day = intval(get_post_meta($item_id, 'duration_day', true));
                    ...
                    $date = date_create(TravelHelper::convertDateformat(STInput::request('check_in')));
                    date_add($date,date_interval_create_from_date_string($duration_day.' '.$day_text));
                    $data['check_out'] = date_format($date,"m/d/Y");

                }else{

                    $check_in = date('m/d/Y',strtotime(get_post_meta($item_id, 'check_in', true )));

                    $data['check_in'] = date('m/d/Y',strtotime(STInput::request('check_in')));

                    $data['check_out'] = date('m/d/Y',strtotime(STInput::request('check_out')));
            ...
            static function get_price_html($post_id=false,$get=false,$st_mid='',$class='')
            {
                ...
                else
                {
                    ...
                    {
                        $html.='<span class="text-small lh1em  onsale">'.TravelHelper::format_money($prices['price_old']).'</span>&nbsp;&nbsp;';
