...
if(!class_exists( 'STUser_f' )) {
    class STUser_f extends TravelerObject
    {
        ...
        function init()
        {
            ...
            //add_action( 'template_redirect' , array( $this , '_get_list_location' ) );
        }

        /**
         *@since 1.1.7
         **/
        function _get_list_location(){
                ...
                switch($sc){
                    case "edit-hotel":
                        ...
                        if(!empty($list_location)){
                            ...
                            $list_location = (empty($list_location))? json_encode("") : json_encode($arr);
                        }
                        break;
                }
        ...
        function dlf_auth( $username , $password )
        {
            ...
            if(empty($username) and empty($password)){
                $status_login = '<div class="error_login">';
                $status_login .= '<strong>ERROR</strong>: The username and password field is empty. ';
                $status_login .= ' </div>';
            }
            if(!is_wp_error( $user )) {
                ...
                if(wp_verify_nonce( $_REQUEST[ 'st_insert_post_hotel' ] , 'user_setting' )) {
                    ...
                    if(!empty( $id_post )) {
                        ...
                        if(isset( $_REQUEST[ 'multi_location' ] )) {
                            ...
                            update_post_meta( $id_post , 'multi_location' , $location_str );
                            update_post_meta( $id_post , 'id_location' , '' );
                        }
                        ...
                        update_post_meta( $id_post , 'is_featured' , STInput::request('is_featured') );
                            ...
                            if(!empty( $_REQUEST[ 'taxonomy' ] )) {
                                ...
                                if(!empty( $taxonomy )) {
                                    ...
                                    foreach( $tax as $key2 => $val2 ) {
                                        wp_set_post_terms( $id_post , $val2 , $key2 );
                                    }
                                }
                            }
                        }
                        ...
                        if(!empty( $custom_field )) {
                            foreach( $custom_field as $k => $v ) {
                                ...
                                update_post_meta( $id_post , $key , STInput::request( $key ) );
                            }
                        }
                        self::$msg = array(
                            'status' => 'success' ,
                ...
                if(wp_verify_nonce( $_REQUEST[ 'st_update_post_hotel' ] , 'user_setting' )) {
                    ...
                    if(!empty( $post_id )) {
                        $my_post = array(
                            ...
                            'post_excerpt' => STInput::request('desc'),
                        );
                        wp_update_post( $my_post );
                        /////////////////////////////////////
                        /// Update Image
                        ...
                        if(!empty($_FILES[ 'logo' ]['name'])){
                            ...
                            update_post_meta($post_id, 'logo', STInput::request('logo'));
                        }
                        ...
                        /// Update meta
                        /////////////////////////////////////
                        update_post_meta( $post_id , 'is_auto_caculate' , $_REQUEST[ 'is_auto_caculate' ] );
                        update_post_meta( $post_id , 'price_avg' , $_REQUEST[ 'price_avg' ] );
        ...
        function st_insert_post_type_room()
        {
            if(!empty( $_REQUEST[ 'btn_insert_post_type_room' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_insert_room' ] , 'user_setting' )) {
                    ...
                    /*$facility_title = $_REQUEST['facility_title'];
                    $facility_value = $_REQUEST['facility_value'];
                    $facility_des = htmlspecialchars($_REQUEST['room_description']);
                    if(empty($facility_title) || empty($facility_value) ||!is_array($facility_title) || !is_array($facility_value)){
                        STTemplate::set_message(__('The facility do not exist ', ST_TEXTDOMAIN), 'danger');
                        return false;
                    }
                    if(count( $facility_title ) && count( $facility_value )) {
                        $facility_arr = array();
                        foreach($facility_title as $key => $val){
                            if(!empty($facility_title[$key])){
                                $facility_arr[] = array(
                                    'title' => $facility_title[$key],
                                    'facility_value' => $facility_value[$key]
                                    );
                            }    
                        }
                    }*/
                    ...
                    if(!empty( $id_post )) {
                        ...
                        if($_REQUEST[ 'external_url' ]) {
                            update_post_meta( $id_post , 'st_room_external_booking' , 'on' );
                            update_post_meta( $id_post , 'st_room_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }

                        //update_post_meta( $id_post , 'add_new_facility' , $facility_arr );
                        //update_post_meta( $id_post , 'room_description' , $facility_des );
            ...
            if(!empty( $_REQUEST[ 'btn_update_post_type_room' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_update_room' ] , 'user_setting' )) {
                    ...
                    if(!empty( $post_id )) {
                        ...
                        /// Update meta
                        /////////////////////////////////////
                        update_post_meta( $post_id , 'room_parent' , $_REQUEST[ 'room_parent' ] );
                        update_post_meta( $post_id , 'number_room' , $_REQUEST[ 'number_room' ] );
                        update_post_meta( $post_id , 'price' , $_REQUEST[ 'price' ] );
                        update_post_meta( $post_id , 'discount_rate' , $_REQUEST[ 'discount_rate' ] );
                        update_post_meta( $post_id , 'adult_number' , $_REQUEST[ 'adult_number' ] );
                        update_post_meta( $post_id , 'children_number' , $_REQUEST[ 'children_number' ] );
                        update_post_meta( $post_id , 'bed_number' , $_REQUEST[ 'bed_number' ] );
                        update_post_meta( $post_id , 'room_footage' , $_REQUEST[ 'room_footage' ] );
                        update_post_meta( $post_id , 'is_featured' , STInput::request('is_featured') );
                        ...
                        if($_REQUEST[ 'external_url' ]) {
                            ...
                            update_post_meta( $post_id , 'st_room_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }
                        /////////////////////////////////////
                        /// Custom Price
                        /////////////////////////////////////
            ...
            if(!empty( $_REQUEST[ 'btn_insert_post_type_tours' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_insert_post_tours' ] , 'user_setting' )) {
                    ...
                    if(!empty( $id_post )) {
                        ...
                        if(!empty( $_REQUEST[ 'is_sale_schedule' ] ))
                            $is_sale_schedule = 'on'; else $is_sale_schedule = 'off';
                        ...
                        if($_REQUEST[ 'external_url' ]) {
                            update_post_meta( $id_post , 'st_tour_external_booking' , 'on' );
                            update_post_meta( $id_post , 'st_tour_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }
            ...
            if(!empty( $_REQUEST[ 'btn_update_post_type_tours' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_update_post_tours' ] , 'user_setting' )) {
                    ...
                    if(!empty( $post_id )) {
                        ...
                        /// Update meta
                        /////////////////////////////////////
                        update_post_meta( $post_id , 'contact_email' , $_REQUEST[ 'email' ] );
                        update_post_meta( $post_id , 'video' , $_REQUEST[ 'video' ] );
                        update_post_meta( $post_id , 'gallery_style' , $_REQUEST[ 'gallery_style' ] );
                        update_post_meta( $post_id , 'address' , $_REQUEST[ 'address' ] );
                        update_post_meta( $post_id , 'map_lat' , $_REQUEST[ 'map_lat' ] );
                        update_post_meta( $post_id , 'map_lng' , $_REQUEST[ 'map_lng' ] );
                        update_post_meta( $post_id , 'map_zoom' , $_REQUEST[ 'map_zoom' ] );
                        update_post_meta( $post_id , 'type_price' , $_REQUEST[ 'type_price' ] );
                        update_post_meta( $post_id , 'price' , $_REQUEST[ 'price' ] );
                        update_post_meta( $post_id , 'adult_price' , $_REQUEST[ 'adult_price' ] );
                        update_post_meta( $post_id , 'child_price' , $_REQUEST[ 'child_price' ] );
                        update_post_meta( $post_id , 'discount' , $_REQUEST[ 'discount' ] );
                        if(!empty( $_REQUEST[ 'is_sale_schedule' ] ))
                            $is_sale_schedule = 'on'; else $is_sale_schedule = 'off';
                        update_post_meta( $post_id , 'is_sale_schedule' , $is_sale_schedule );
                        update_post_meta( $post_id , 'sale_price_from' , $_REQUEST[ 'sale_price_from' ] );
                        update_post_meta( $post_id , 'sale_price_to' , $_REQUEST[ 'sale_price_to' ] );
                        update_post_meta( $post_id , 'type_tour' , $_REQUEST[ 'tour_type' ] );
                        update_post_meta( $post_id , 'check_in' , $_REQUEST[ 'check_in' ] );
                        update_post_meta( $post_id , 'check_out' , $_REQUEST[ 'check_out' ] );
                        update_post_meta( $post_id , 'max_people' , $_REQUEST[ 'max_people' ] );
                        update_post_meta( $post_id , 'duration_day' , $_REQUEST[ 'duration' ] );
                        update_post_meta( $post_id , 'is_featured' , STInput::request('is_featured') );
                        if($_REQUEST[ 'external_url' ]) {
                            update_post_meta( $post_id , 'st_tour_external_booking' , 'on' );
                            update_post_meta( $post_id , 'st_tour_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }
                        if(!empty( $_REQUEST[ 'program_title' ] )) {
                            $program_title = $_REQUEST[ 'program_title' ];
                            $program_desc  = $_REQUEST[ 'program_desc' ];
            ...
            if(!empty( $_REQUEST[ 'btn_insert_post_type_activity' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_insert_post_activity' ] , 'user_setting' )) {
                    ...
                    if(!empty( $id_post )) {
                        ...
                        update_post_meta( $id_post , 'type_activity' , $_REQUEST[ 'type_activity' ] );
                        update_post_meta( $id_post , 'check_in' , $_REQUEST[ 'check_in' ] );
                        update_post_meta( $id_post , 'check_out' , $_REQUEST[ 'check_out' ] );
                        ...
                        if($_REQUEST[ 'external_url' ]) {
                            update_post_meta( $id_post , 'st_activity_external_booking' , 'on' );
                            update_post_meta( $id_post , 'st_activity_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }
            ...
            if(!empty( $_REQUEST[ 'btn_update_post_type_activity' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_update_post_activity' ] , 'user_setting' )) {
                    ...
                    if(!empty( $post_id )) {
                        ...
                        /////////////////////////////////////
                        update_post_meta( $post_id , 'contact_email' , $_REQUEST[ 'email' ] );
                        update_post_meta( $post_id , 'contact_web' , $_REQUEST[ 'website' ] );
                        update_post_meta( $post_id , 'contact_phone' , $_REQUEST[ 'phone' ] );
                        update_post_meta( $post_id , 'video' , $_REQUEST[ 'video' ] );
                        update_post_meta( $post_id , 'gallery_style' , $_REQUEST[ 'gallery_style' ] );
                        update_post_meta( $post_id , 'address' , $_REQUEST[ 'address' ] );
                        update_post_meta( $post_id , 'map_lat' , $_REQUEST[ 'map_lat' ] );
                        update_post_meta( $post_id , 'map_lng' , $_REQUEST[ 'map_lng' ] );
                        update_post_meta( $post_id , 'map_zoom' , $_REQUEST[ 'map_zoom' ] );
                        update_post_meta( $post_id , 'type_activity' , $_REQUEST[ 'type_activity' ] );
                        update_post_meta( $post_id , 'check_in' , $_REQUEST[ 'check_in' ] );
                        update_post_meta( $post_id , 'check_out' , $_REQUEST[ 'check_out' ] );
                        update_post_meta( $post_id , 'activity-time' , $_REQUEST[ 'activity-time' ] );
                        update_post_meta( $post_id , 'duration' , $_REQUEST[ 'duration' ] );
                        update_post_meta( $post_id , 'venue-facilities' , $_REQUEST[ 'venue-facilities' ] );
                        update_post_meta( $post_id , 'type_price' , $_REQUEST[ 'type_price' ] );
                        update_post_meta( $post_id , 'price' , $_REQUEST[ 'price' ] );
                        update_post_meta( $post_id , 'adult_price' , $_REQUEST[ 'adult_price' ] );
                        update_post_meta( $post_id , 'child_price' , $_REQUEST[ 'child_price' ] );
                        ...
                        update_post_meta( $post_id , 'is_featured' , STInput::request('is_featured') );
                        if($_REQUEST[ 'external_url' ]) {
                            update_post_meta( $post_id , 'st_activity_external_booking' , 'on' );
                            update_post_meta( $post_id , 'st_activity_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }

                        if($_REQUEST[ 'best-price-guarantee' ]) {
                            update_post_meta( $post_id , 'best-price-guarantee' , 'on' );
                        } else {
                            update_post_meta( $post_id , 'best-price-guarantee' , 'off' );
                        }
                        update_post_meta( $post_id , 'best-price-guarantee-text' , $_REQUEST[ 'best-price-guarantee-text' ] );
                        update_post_meta( $post_id , 'rate_review' , '1' );
                        /////////////////////////////////////
                        /// Update taxonomy
                        /////////////////////////////////////
            ...
            if(!empty( $_REQUEST[ 'btn_insert_post_type_cars' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_insert_post_cars' ] , 'user_setting' )) {
                    ...
                    if(!empty( $id_post )) {
                        ...
                        $logo_tmp = $logo_tmp[0];
                        update_post_meta( $id_post , 'cars_logo' ,$logo_tmp );
                        ...
                        if(!empty($_FILES[ 'gallery' ]['name'][0])){
                            ...
                            update_post_meta( $id_post , 'gallery' , STInput::request('id_gallery') );
                        }
                        ...
                        update_post_meta( $id_post , 'is_featured' , $_REQUEST[ 'is_featured' ] );
                        if($_REQUEST[ 'external_url' ]) {
                            update_post_meta( $id_post , 'st_car_external_booking' , 'on' );
                            update_post_meta( $id_post , 'st_car_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }
                        ...
                        if(!empty( $_REQUEST[ 'features_taxonomy' ] )) {
                            $features        = array();
                            $features_taxonomy = $_REQUEST[ 'features_taxonomy' ];
                            $features_taxonomy_info = $_REQUEST[ 'features_taxonomy_info' ];
                ...
                if(wp_verify_nonce( $_REQUEST[ 'st_update_post_cars' ] , 'user_setting' )) {
                    ...
                    if(!empty( $post_id )) {
                        ...
                        if(!empty($_FILES[ 'logo' ]['name'])){
                            ...
                            update_post_meta($post_id, 'cars_logo',STInput::request('logo'));
                        }
                        if(!empty($_FILES[ 'gallery' ]['name'][0])){
                            $gallery = $_FILES[ 'gallery' ];
                            if(!empty( $gallery )) {
                        ...
                        /// Update meta
                        /////////////////////////////////////
                        update_post_meta( $post_id , 'cars_name' , $_REQUEST[ 'st_name' ] );
                        update_post_meta( $post_id , 'cars_email' , $_REQUEST[ 'email' ] );
                        update_post_meta( $post_id , 'cars_phone' , $_REQUEST[ 'phone' ] );
                        update_post_meta( $post_id , 'cars_about' , $_REQUEST[ 'about' ] );
                        update_post_meta( $post_id , 'video' , $_REQUEST[ 'video' ] );
                        update_post_meta( $post_id , 'cars_address' , $_REQUEST[ 'address' ] );
                        update_post_meta( $post_id , 'map_lat' , $_REQUEST[ 'map_lat' ] );
                        update_post_meta( $post_id , 'map_lng' , $_REQUEST[ 'map_lng' ] );
                        update_post_meta( $post_id , 'map_zoom' , $_REQUEST[ 'map_zoom' ] );
                        update_post_meta( $post_id , 'cars_price' , $_REQUEST[ 'price' ] );
                        update_post_meta( $post_id , 'discount' , $_REQUEST[ 'discount' ] );
                        update_post_meta( $post_id , 'is_featured' , STInput::request('is_featured') );
                        if($_REQUEST[ 'external_url' ]) {
                            update_post_meta( $post_id , 'st_car_external_booking' , 'on' );
                            update_post_meta( $post_id , 'st_car_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }
                        /////////////////////////////////////
                        /// Update equipment item
                        /////////////////////////////////////
                ...
                if(wp_verify_nonce( $_REQUEST[ 'st_insert_post_rental' ] , 'user_setting' )) {
                    ...
                    if(!empty( $id_post )) {
                        ...
                        if(isset( $_REQUEST[ 'multi_location' ] )) {
                            ...
                            update_post_meta( $id_post , 'multi_location' , $location_str );
                            update_post_meta( $id_post , 'location_id' , '' );
                        }
                        ...
                        update_post_meta( $id_post , 'rental_max_adult' , $_REQUEST[ 'rental_max_adult' ] );
                        update_post_meta( $id_post , 'rental_max_children' , $_REQUEST[ 'rental_max_children' ] );
                        update_post_meta( $id_post , 'rentals_booking_period' , $_REQUEST[ 'rentals_booking_period' ] );
                        update_post_meta( $id_post , 'is_featured' , STInput::request('is_featured') );

                        if($_REQUEST[ 'external_url' ]) {
                            update_post_meta( $id_post , 'st_rental_external_booking' , 'on' );
                            update_post_meta( $id_post , 'st_rental_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }
            ...
            if(!empty( $_REQUEST[ 'btn_update_post_type_rental' ] )) {
                if(wp_verify_nonce( $_REQUEST[ 'st_update_post_rental' ] , 'user_setting' )) {
                    ...
                    if(!empty( $post_id )) {
                        ...
                        /////////////////////////////////////
                        update_post_meta( $post_id , 'address' , $_REQUEST[ 'address' ] );
                        update_post_meta( $post_id , 'agent_email' , $_REQUEST[ 'email' ] );
                        update_post_meta( $post_id , 'agent_website' , $_REQUEST[ 'website' ] );
                        update_post_meta( $post_id , 'agent_phone' , $_REQUEST[ 'phone' ] );
                        update_post_meta( $post_id , 'video' , $_REQUEST[ 'video' ] );
                        update_post_meta( $post_id , 'map_lat' , $_REQUEST[ 'map_lat' ] );
                        update_post_meta( $post_id , 'map_lng' , $_REQUEST[ 'map_lng' ] );
                        update_post_meta( $post_id , 'map_zoom' , $_REQUEST[ 'map_zoom' ] );
                        ...
                        update_post_meta( $post_id , 'rental_max_adult' , $_REQUEST[ 'rental_max_adult' ] );
                        update_post_meta( $post_id , 'rental_max_children' , $_REQUEST[ 'rental_max_children' ] );
                        update_post_meta( $post_id , 'rentals_booking_period' , $_REQUEST[ 'rentals_booking_period' ] );
                        update_post_meta( $post_id , 'is_featured' , STInput::request('is_featured') );
                        if($_REQUEST[ 'external_url' ]) {
                            update_post_meta( $post_id , 'st_rental_external_booking' , 'on' );
                            update_post_meta( $post_id , 'st_rental_external_booking_link' , $_REQUEST[ 'external_url' ] );
                        }
                        /////////////////////////////////////
                        /// Update taxonomy
                        /////////////////////////////////////
                ...
                if(wp_verify_nonce( $_REQUEST[ 'st_insert_rental_room' ] , 'user_setting' )) {
                    ...
                    if(!empty( $id_post )) {
                        ...
                        if(!empty($_FILES[ 'featured-image' ]['name'])){
                           ...
                           // update_post_meta($id_post, '_thumbnail_id', STInput::request('id_featured_image'));
                        }
                        if(!empty($_FILES[ 'gallery' ]['name'][0])){
                            $gallery = $_FILES[ 'gallery' ];
                            if(!empty( $gallery )) {
                        ...
                        update_post_meta( $id_post , 'bed_number' , $_REQUEST[ 'bed_number' ] );
                        update_post_meta( $id_post , 'room_footage' , $_REQUEST[ 'room_footage' ] );
                        update_post_meta( $id_post , 'room_description' , $_REQUEST[ 'room_description' ] );
                        update_post_meta( $id_post , 'gallery_style' , $_REQUEST[ 'gallery_style' ] );
                        /////////////////////////////////////
                        /// Update taxonomy
                        /////////////////////////////////////
        ...
        function get_book_history( $type = '' )
        {
            ...
            while( have_posts() ) {
                ...
                if(STReview::check_reviewable( $comment_post_id ) and comments_open( $comment_post_id )) {
                    $action = '<a href="' . st_get_link_with_search( get_permalink( $user_url ) , array(
                            'sc' ,
                            'item_id'
