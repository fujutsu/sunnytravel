    ...
    class STActivity extends TravelerObject
    {
        ...
        function _alter_search_query($where)
        {
            ...
            $where.=" OR {$wpdb->posts}.ID in
                            (SELECT post_id from $wpdb->postmeta where meta_key='type_activity'
                            and meta_value='daily_activity')";
        ...
        function activity_add_to_cart()
        {
            if(STInput::get( 'action' ) == 'activity_add_to_cart') {
                ...
                $data[ 'activity_time' ] = STInput::get( 'activity_time' );
                $data[ 'type_price' ]    = STInput::request( 'type_price' );
                $data[ 'type_activity' ] = STInput::request( 'type_activity' );
                ...
                if(STInput::request( 'type_activity' ) == 'specific_date') {
                    $check_in = date('m/d/Y',strtotime(STInput::get( 'activity_start' )));
                    $check_out = date('m/d/Y',strtotime(STInput::get( 'activity_end' )));
                ...
                } elseif(STInput::request( 'type_activity' ) == 'daily_activity'){

                    $check_in = STInput::get('start', date(TravelHelper::getDateFormat()));
                    $check_in = TravelHelper::convertDateFormat($check_in);
                    $data[ 'check_in' ] = $check_in;
                    $data[ 'check_out' ]     = TravelHelper::convertDateFormat(STInput::get( 'end' ));
                    $today = date('m/d/Y');
        ...
        function do_add_to_cart()
        {
            ...
            $data[ 'check_in' ]      = STInput::request( 'start' );
            $data[ 'check_out' ]     = STInput::request( 'end' );
            $data[ 'activity_time' ] = STInput::get( 'activity_time' );
            $data[ 'type_price' ]    = STInput::request( 'type_price' );
            $data[ 'type_activity' ] = STInput::request( 'type_activity' );
