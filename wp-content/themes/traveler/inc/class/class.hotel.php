    ...
    class STHotel extends TravelerObject
    {
        ...
        static function get_search_fields_name()
        {
            return array(
                ...
                'location_tax'       => array(
                    'value' => 'location_tax',
                    'label' => __('Location Taxonomy', ST_TEXTDOMAIN)
                ),
                'list_location_tax'       => array(
                    'value' => 'list_location_tax',
                    'label' => __('Location Taxonomy List', ST_TEXTDOMAIN)
                ),
                'checkin'       => array(
                    'value' => 'checkin',
        ...
        function hotel_add_to_cart()
        {
            if (STInput::request('action') == 'hotel_add_to_cart') {
                ...
                $data_price = STRoom::get_room_price($room_id,$check_in,$check_out);

                $price = $data_price['price'] * $room_num_search;
        ...
        function do_add_to_cart($array = array())
        {
            ...
            $year_checkin = date('Y',strtotime($check_in));
            $list_checkin = TravelHelper::getOrderByYear_2($year_checkin, 'hotel_room', 'st_hotel', $data['room_id']);

            $year_checkout = date('Y',strtotime($check_out));
            $list_checkout = TravelHelper::getOrderByYear_2($year_checkout, 'hotel_room', 'st_hotel', $data['room_id']);

            $list_in_out = TravelHelper::getListDate(date('Y/m/d',strtotime($check_in)), date('Y/m/d',strtotime($check_out)));

            if(is_array($list_in_out) and count($list_in_out)){
                foreach($list_in_out as $inout){
                    if(in_array($inout, $list_checkin) or in_array($inout, $list_checkout)){
                        STTemplate::set_message(__('This date has been ordered', ST_TEXTDOMAIN), 'danger');
                        $pass_validate = FALSE;
                        break;
                    }
                }
            ...
            if(strtotime($check_out) - strtotime($check_in) <= 0){
                STTemplate::set_message(__('Minimum order is 1 day at present.', ST_TEXTDOMAIN), 'danger');
                $pass_validate = FALSE;
            }
...
SELECT st_meta6.meta_value as booked_id ,st_meta2.meta_value as check_in,st_meta3.meta_value as check_out
        ...
        function ajax_search_room()
        {
            if (st_is_ajax() and STInput::post('room_search')) {
                ...
                $year_checkin = date('Y',$check_in);

                $year_checkout = date('Y',$check_out);
                $list_in_out = TravelHelper::getListDate(date('Y/m/d',$check_in), date('Y/m/d',$check_out));
                if (have_posts()) {
                    while (have_posts()) {
                        the_post();

                        $list_checkin = TravelHelper::getOrderByYear_2($year_checkin, 'hotel_room', 'st_hotel', get_the_ID());

                        $list_checkout = TravelHelper::getOrderByYear_2($year_checkout, 'hotel_room', 'st_hotel', get_the_ID());
                        if(is_array($list_in_out) and count($list_in_out)){
                            $status = true;
                            $i = 0;
                            foreach($list_in_out as $inout){
                    ...
                    if($i == 0){
                        $result = array(
                            'status'    => 0,
                            'data'      => "",
                            'message'   => __('No available room', ST_TEXTDOMAIN),
                        );
        ...
        {
            if (is_admin() and empty( $_REQUEST['is_search_map'] )) return FALSE;
            ...
            if ($query->is_search && $post_type == 'st_hotel') {
                ...
                if(STInput::get('location_tax_id'))
                {
                    $tax_query[]=array(
                        'taxonomy'=>'st_location_tax',
                        'terms'     =>array(STInput::get('location_tax_id'))
                    );
                }
                ...
                if ($location_id = STInput::request('location_id')) {

                    $meta_query[ ] = array(
                        array(
                            'key'     => 'multi_location' ,
                            'value'   => "_{$location_id}_" ,
                            'compare' => "LIKE" ,
                        ),
                        array(
                            'key'     => 'id_location' ,
                ...
                if ($star = STInput::get('star_rate')) {
                    ...
                    $min_star = intval($min_star);
                    $list_min_star = array();
                    for($i = $min_star; $i< $min_star+ 0.9; $i+=0.1){
                        $list_min_star[] = $i;
                    }
