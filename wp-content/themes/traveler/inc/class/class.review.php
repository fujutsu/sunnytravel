...
if(!class_exists('STReview'))
{
    class STReview
    {
        ...
        * from 1.1.7
        */
        static function review_check_final(){

            // theme option
            $review_need_booked = st()->get_option('review_need_booked');
            $review_without_login = st()->get_option('review_without_login') ;  
            $review_once = st()->get_option('review_once');   

            // check reviewed
            $check_reviewable = comments_open();
            $check_reviewed = self::check_reviewed();
            
            if (!is_user_logged_in() and $review_without_login == "off"){                
                return "must_login"; 
            }
            ...
            if ($check_reviewed and $review_once=="off"){
                return "reviewed" ; 
            }            
            return "true";
